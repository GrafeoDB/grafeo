"""Cypher implementation of algorithm benchmarks.

Benchmarks graph algorithms using db.algorithms.* API.
Cypher is used for setup only.
"""
...
class BenchCypherAlgorithms(BaseBenchAlgorithms):
    """Cypher implementation of algorithm benchmarks.

    Note: Algorithms are accessed via db.algorithms.*, not via Cypher queries.
    Cypher is used for graph setup only.
    """
