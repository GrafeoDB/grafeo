"""GQL implementation of algorithm benchmarks.

Benchmarks graph algorithms using db.algorithms.* API.
GQL is used for setup only.
"""

import random
from tests.python.bases.bench_algorithms import BaseBenchAlgorithms


class BenchGQLAlgorithms(BaseBenchAlgorithms):
    """GQL implementation of algorithm benchmarks.
    ...
    def run_dijkstra(self, db, source, target=None, weight_prop: str = "weight"):
        ...
        if target is None:
            return db.algorithms.dijkstra_all(source, weight_prop)
