...
class TestGQLNetworkXComparison(BaseNetworkXComparisonTest):
    """GQL implementation of NetworkX comparison tests.

    Note: Algorithms are accessed via db.algorithms.*, not via GQL queries.
    GQL is used for graph setup and verification only.
    """
    ...
    def setup_random_graph(
        ...
        while len(edges) < n_edges:
            ...
            if src != dst and (src, dst) not in edge_set:
                ...
                db.create_edge(src, dst, "EDGE", {"weight": weight} if weighted else {})
...
class TestGQLNetworkXBenchmark(BaseNetworkXBenchmarkTest):
    """GQL implementation of NetworkX performance comparison tests.
