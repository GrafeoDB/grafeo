"""Gremlin implementation of NetworkX comparison tests.

Compares Grafeo algorithm results against NetworkX to verify correctness.
Note: These tests use Python API only, they don't require Gremlin support.
"""
...
from tests.python.bases.test_networkx import (
...
@pytest.fixture
def db():
    """Create a fresh database instance (no Gremlin required)."""
    return GrafeoDB()


class TestGremlinNetworkXComparison(BaseNetworkXComparisonTest):
    """Gremlin implementation of NetworkX comparison tests.
...
class TestGremlinNetworkXBenchmark(BaseNetworkXBenchmarkTest):
    """Gremlin implementation of NetworkX performance comparison tests.
